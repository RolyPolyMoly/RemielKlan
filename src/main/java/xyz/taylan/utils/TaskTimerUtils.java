            ...
            public void run() {
                if (time == 1){
                    try {
                        ...
                        logger.info(ColorUtils.translateColorCodes(messagesConfig.getString("otomatik-kayit-tamamlandı")));
                    } catch (IOException e) {
                        logger.severe(ColorUtils.translateColorCodes(messagesConfig.getString("otomatik-kayit-başarısız")));
                        e.printStackTrace();
                    }
                    runClansAutoSaveTwo();
